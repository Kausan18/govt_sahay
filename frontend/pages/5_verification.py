...
.result-verified { background:#F0FAF0; border:2px solid #138808; border-radius:20px; padding:40px 36px; text-align:center; margin-bottom:20px; }
.result-failed   { background:#FFF4F4; border:2px solid #E74C3C; border-radius:20px; padding:40px 36px; text-align:center; margin-bottom:20px; }
...
.face-banner-ok   { background:#E8F7E8; border:1.5px solid #138808; border-radius:14px; padding:14px 20px; margin-bottom:16px; display:flex; align-items:center; gap:12px; }
.face-banner-warn { background:#FFF8EE; border:1.5px solid #FF9933; border-radius:14px; padding:14px 20px; margin-bottom:16px; display:flex; align-items:center; gap:12px; }
.banner-icon { font-size:1.3rem; flex-shrink:0; }
.banner-title { font-family:'Plus Jakarta Sans',sans-serif; font-size:0.84rem; font-weight:600; }
.banner-sub   { font-family:'Plus Jakarta Sans',sans-serif; font-size:0.78rem; margin-top:2px; }
.banner-ok-title   { color:#0B6E04; }
.banner-ok-sub     { color:#1D8A14; }
.banner-warn-title { color:#8B5E1A; }
.banner-warn-sub   { color:#8B5E1A; }
...
.doc-check-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:0; }
.dc-name  { font-family:'Plus Jakarta Sans',sans-serif; font-size:0.92rem; font-weight:600; color:#1C1C1A; }
.dc-issue { font-family:'Plus Jakarta Sans',sans-serif; font-size:0.78rem; color:#C0392B; margin-top:3px; }
.badge-pass { background:#E8F7E8; color:#138808; font-family:'Plus Jakarta Sans',sans-serif; font-size:0.72rem; font-weight:600; padding:4px 12px; border-radius:20px; white-space:nowrap; }
.badge-fail { background:#FFF0F0; color:#C0392B; font-family:'Plus Jakarta Sans',sans-serif; font-size:0.72rem; font-weight:600; padding:4px 12px; border-radius:20px; white-space:nowrap; }

/* sub-check rows inside aadhaar card */
.sub-checks { margin-top:12px; border-top:1px solid #F0F0EB; padding-top:12px; display:flex; flex-direction:column; gap:8px; }
.sub-row { display:flex; align-items:flex-start; gap:10px; }
.sub-icon { font-size:0.9rem; flex-shrink:0; margin-top:1px; }
.sub-label { font-family:'Plus Jakarta Sans',sans-serif; font-size:0.82rem; font-weight:600; }
.sub-detail { font-family:'Plus Jakarta Sans',sans-serif; font-size:0.76rem; margin-top:2px; }
.sub-ok   { color:#138808; }
.sub-fail { color:#C0392B; }
...
if face_verified is True:
    st.markdown(
        '<div class="face-banner-ok">'
        '<div class="banner-icon">🧑</div>'
        '<div><div class="banner-title banner-ok-title">Face verification passed</div>'
        '<div class="banner-sub banner-ok-sub">Live photo matched your profile photo successfully</div>'
        '</div></div>',
        unsafe_allow_html=True
    )
elif face_verified is False:
    st.markdown(
        '<div class="face-banner-warn">'
        '<div class="banner-icon">⚠️</div>'
        '<div><div class="banner-title banner-warn-title">Face verification skipped or failed</div>'
        '<div class="banner-sub banner-warn-sub">Document verification still proceeded — consider retrying the face check</div>'
        '</div></div>',
        unsafe_allow_html=True
    )
...
if verified:
    ...
    reason_items = "".join(
        f'<li style="margin-bottom:6px">{r}</li>' for r in (missing + issues)
    )
    st.markdown(
        f'<div class="result-failed">'
        f'<div class="r-icon">⚠️</div>'
        f'<div class="r-title fail">Verification incomplete</div>'
        f'<div class="r-sub fail">'
        f'<ul style="text-align:left;margin-top:12px;padding-left:20px;line-height:1.8">'
        f'{reason_items}</ul></div></div>',
        unsafe_allow_html=True
    )
...
if doc_results:
    ...
    for doc_type, res in doc_results.items():
        ...
        sub_checks  = res.get("sub_checks", {})   # NEW — per-field results
        overall_badge = '<span class="badge-pass">✓ Passed</span>' if passed else '<span class="badge-fail">✗ Failed</span>'

        # Build sub-check rows (shown for aadhaar; empty for income/caste)
        sub_html = ""
        if sub_checks:
            rows = []
            for key, sc in sub_checks.items():
                sc_passed = sc.get("passed", False)
                icon      = "✓" if sc_passed else "✗"
                cls       = "sub-ok" if sc_passed else "sub-fail"
                label     = sc.get("label", key)
                detail    = sc.get("detail", "")
                rows.append(
                    f'<div class="sub-row">'
                    f'<div class="sub-icon {cls}">{icon}</div>'
                    f'<div>'
                    f'<div class="sub-label {cls}">{label}</div>'
                    f'<div class="sub-detail {cls}">{detail}</div>'
                    f'</div></div>'
                )
            sub_html = f'<div class="sub-checks">{"".join(rows)}</div>'

        # Fallback: show issues if no sub_checks (income, caste docs)
        issues_html = ""
        if not sub_checks and doc_issues:
            issues_html = "".join(f'<div class="dc-issue">{i}</div>' for i in doc_issues)

        st.markdown(
            f'<div class="doc-check">'
            f'<div class="doc-check-header">'
            f'<div class="dc-name">{doc_type.replace("_", " ").title()}</div>'
            f'{overall_badge}'
            f'</div>'
            f'{issues_html}'
            f'{sub_html}'
...
if missing:
    ...
    for doc in missing:
        st.markdown(
            f'<div class="doc-check">'
            f'<div class="doc-check-header">'
            f'<div class="dc-name">{doc.replace("_"," ").title()}</div>'
            f'<span class="badge-fail">Not uploaded</span>'
            f'</div></div>',
            unsafe_allow_html=True
        )
...
if verified:
    ...
    if scheme.get("official_url"):
        portal_html = (
            f'<br><br><a href="{scheme["official_url"]}" target="_blank" '
            f'style="color:#FF9933;font-weight:600">Visit official portal ↗</a>'
        )
    st.markdown(
        f'<div class="next-card">'
        f'<div class="next-title">Apply on the official portal</div>'
        f'<div class="next-sub">Your documents are verified. Head to the government portal with your documents ready.{portal_html}</div>'
        f'</div>',
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        ...
        '<div class="next-sub">Resolve the issues listed above, then re-verify. '
        'You can upload missing documents from the scheme detail page.</div>'
        '</div>',
        unsafe_allow_html=True
    )
        ...
        with st.spinner("AI is explaining what went wrong…"):
            try:
                r = call("/ai/ask-scheme", {
                    "scheme_id": st.session_state["verify_scheme_id"],
                    "question": (
                        f"Verification failed for these reasons: {', '.join(missing + issues)}. "
                        f"Explain in simple, friendly language what the user needs to fix and the exact steps."
                    ),
                    "language": "English"
                })
    ...
    if st.session_state.get("vai_explain"):
        st.markdown(
            f'<div class="ai-explain">'
            f'<div class="ai-explain-label">AI Guidance</div>'
            f'<div class="ai-explain-text">{st.session_state["vai_explain"]}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
